<?php 
    ...
    $admin = mysqli_fetch_all(mysqli_query($db , "SELECT * FROM admin"),  MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="links d-none d-md-block">
        <nav class="navbar">
            ...
            <li class="nav-item rounded-pill"><a class="nav-link" href="dashboard.php"><i class="fa-solid fa-chart-line"></i> Dashboard</a></li>
                ...
                <li class="nav-item rounded-pill active"><a class="nav-link" href="administration.php"><i class="fa-solid fa-school"></i> Administration</a></li>
            ...
            <table class="table text-white table-striped table-sm">
                ...
                <tbody>
                    <?php foreach($admin as $admins): ?>
                        <tr>
                            <td class="text-white"><?=  $admins['CIN'] ?></td>
                            <td class="text-white"><?= $admins['Nom'] ?></td>
                            <td class="text-white"><?= $admins['Prenom'] ?></td>
                            <td class="text-white"><?= $admins['Email']?></td>
                            <td class="text-white">
                                <a href="../php/modif-admin.php?id=<?= $admins['id'] ?>"
                                id="ajout-btn" type="button"  class="btn main-btn rounded-pill mr-2">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a onclick="return confirm('Voulez-vous vraimet supprimer <?= $admins['Prenom'].' '.$admins['Nom'] ?> ?')"
                                    href="../php/supp-admin.php?id=<?= $admins['id'] ?>"
                                    class='btn cancel-btn rounded-pill mr-2'>
